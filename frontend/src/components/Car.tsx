...
const Car: React.FC<CarProps> = ({
  ...
  useFrame((_, delta) => {
    ...
    if (associatedLightState === 'green') {
      if (movementAxis === 'x') {
        ...
        if ( (movementDirection === 1 && car.position.x >= endPositionVec.x) ||
             (movementDirection === -1 && car.position.x <= endPositionVec.x) ) {
          car.position.copy(startPositionVec);
        }
      } else { // movementAxis === 'z'
        car.position.z += effectiveSpeed * movementDirection;
        if ( (movementDirection === 1 && car.position.z >= endPositionVec.z) ||
             (movementDirection === -1 && car.position.z <= endPositionVec.z) ) {
          car.position.copy(startPositionVec);
        }
      }
    } else { // Light is 'red'
      if (movementAxis === 'x') {
        ...
        if ( (movementDirection === 1 && currentX < stopX) || 
             (movementDirection === -1 && currentX > stopX) ) {
          let newX = currentX + effectiveSpeed * movementDirection;
          if ((movementDirection === 1 && newX >= stopX) || (movementDirection === -1 && newX <= stopX)) {
            newX = stopX;
          }
          car.position.x = newX;
        } else if (car.position.x !== stopX) {
            car.position.x = stopX;
        }
      } else { // movementAxis === 'z'
        const currentZ = car.position.z;
        const stopZ = stopPositionVec.z;
         if ( (movementDirection === 1 && currentZ < stopZ) || 
             (movementDirection === -1 && currentZ > stopZ) ) {
          let newZ = currentZ + effectiveSpeed * movementDirection;
          if ((movementDirection === 1 && newZ >= stopZ) || (movementDirection === -1 && newZ <= stopZ)) {
            newZ = stopZ;
          }
          car.position.z = newZ;
        } else if (car.position.z !== stopZ) {
            car.position.z = stopZ;
        }
      }
    }
  });
  ...
  return (
    <mesh ref={carRef} position={startPositionVec} rotation={rotationEulers}>
